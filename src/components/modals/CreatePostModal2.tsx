...
const CreatePostModal = (): JSX.Element => {
    ...
    const [textUrls, setTextUrls] = useState<any[]>([])
    ...
    const [urlDataOld, setUrlDataOld] = useState<any>(null)
    ...
    function scrapeURL(urlString) {
        axios
            ...
            .then((res) => {
                console.log(res)
                setUrlData((ud) => [...ud, { url: urlString, ...res.data }])
            })
            .catch((error) => console.log(error))
    }
    ...
    function findPostData() {
        ...
        if (postType === 'Url') {
            ...
            data.urlImage = urlDataOld.image || null
            data.urlDomain = urlDataOld.domain || null
            data.urlTitle = urlDataOld.title || null
            data.urlDescription = urlDataOld.description || null
        }
        if (postType === 'Image') {
        ...
        if (postType === 'String') {
            ...
            data.StringPosts = string.map((bead, index) => {
                ...
                return {
                    ...
                    color: bead.color,
                    url: beadUrl,
                    urlDescription: bead.urlData ? bead.urlDataOld.description : null,
                    urlDomain: bead.urlData ? bead.urlDataOld.domain : null,
                    urlImage: bead.urlData ? bead.urlDataOld.image : null,
                    urlTitle: bead.urlData ? bead.urlDataOld.title : null,
                    Link: { index },
    ...
    function formatBeadData(bead, index) {
        return {
            ...
            urlName: bead.urlData ? bead.urlDataOld.name : null,
            urlDomain: bead.urlData ? bead.urlDataOld.domain : null,
            urlTitle: bead.urlData ? bead.urlDataOld.title : null,
            urlImage: bead.urlData ? bead.urlDataOld.image : null,
            Link: { index },
    ...
    useEffect(() => {
        if (textUrls.length) {
            console.log('URLs: ', textUrls)
            textUrls.forEach((url) => {
                if (!urlData.find((ud) => ud.url === url)) scrapeURL(url)
            })
        }
    }, [textUrls])
                    ...
                    {postType === 'Text' && currentStep === 2 && (
                        <Column centerX style={{ width: '100%', maxWidth: 500, marginBottom: 30 }}>
                            <DraftTextEditor
                                ...
                                maxChars={5000}
                                onChange={(value, userMentions, urls) => {
                                    setTextForm({
                                        ...textForm,
                                    ...
                                    {urlData && (
                                        <Column className={styles.urlPreviewWrapper}>
                                            <UrlPreview
                                                ...
                                                image={urlDataOld.image}
                                                domain={urlDataOld.domain}
                                                title={urlDataOld.title}
                                                description={urlDataOld.description}
                                            />
                                        </Column>
                                    )}
                                </Column>
                                        ...
                                        {newBead.type === 'url' && (
                                            <Column centerX style={{ width: '100%' }}>
                                                ...
                                                {newBead.urlData && (
                                                    <UrlPreview
                                                        url={newBead.url}
                                                        image={newBead.urlDataOld.image}
                                                        domain={newBead.urlDataOld.domain}
                                                        title={newBead.urlDataOld.title}
                                                        description={newBead.urlDataOld.description}
                                                        style={{ marginTop: 10 }}
                                                    />
                                                )}
                                            </Column>
