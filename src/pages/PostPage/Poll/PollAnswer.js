...
function PollAnswer(props) {
    const { answer } = props
    const { postData, selectedPollAnswers, setSelectedPollAnswers, setVoteCast } = useContext(
        PostContext
    )
    ...
    function selectAnswer(value) {
        ...
        if (postData.subType === 'multiple-choice') {
            if (alreadySelected) {
                setSelectedPollAnswers([
                    ...selectedPollAnswers.filter((pollAnswer) => pollAnswer.id !== answer.id),
                ])
            } else {
    ...
    useEffect(() => {
        if (alreadySelected) {
            setAnswerValue(selectedAnswer.value)
        }
    }, [])

    useEffect(() => {
        if (selectedPollAnswers.length < 1) {
            setAnswerValue('')
        }
    }, [selectedPollAnswers])
    ...
    return (
        <div
            className={`${styles.pollAnswer} ${alreadySelected && styles.answerSelected}`}
            onClick={() => {
                if (postData.subType !== 'weighted-choice') selectAnswer()
            }}
        >
            {postData.subType !== 'weighted-choice' && (
                <img
                    ...
                    src={
                        alreadySelected
                            ? '/icons/check-circle-regular.svg'
