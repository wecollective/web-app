...
function PollAnswerForm(props) {
    ...
    function removePollAnswer(pollAnswer) {
        const updatedPollAnswers = pollAnswers.filter((answer) => {
    ...
    return (
        <div className={styles.pollAnswerForm}>
            ...
            <div className={styles.pollAnswerFormTop}>
                <textarea
                    className={`wecoInput textArea mb-10 ${newPollAnswerError && 'error'}`}
                    style={{ height: 40 }}
                    type='text'
                    placeholder='Answer... (max 2,000 characters)'
                    value={newPollAnswer}
                    onChange={(e) => {
                        setNewPollAnswer(e.target.value)
                        setNewPollAnswerError(false)
                    }}
                />
                <div
                    className={styles.pollAnswerFormButton}
                    onClick={() => {
                        addPollAnswer(newPollAnswer)
                    }}
                >
                    Add answer
                </div>
